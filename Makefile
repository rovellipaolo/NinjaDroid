...
.PHONY: test-coverage
test-coverage:
	@pipenv run coverage run --branch --source=. --omit="tests/*,regression/*" --data-file=".coverage" -m unittest
	@pipenv run coverage xml --data-file=".coverage" -o "coverage.xml"
	@pipenv run coverage report --data-file=".coverage" --show-missing
