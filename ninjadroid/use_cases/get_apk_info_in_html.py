...
class GetApkInfoInHtml:
    ...
    def execute(self, apk: APK, input_filename: str,  output_directory: str):
        self.logger.info("Generating HTML report file...")
        ...
        with open(os.path.join(output_directory, report_filename), "w") as file:
            apk_info = self.generate_html_report(apk)
            file.write(apk_info)

    @staticmethod
    def generate_html_report(apk: APK) -> str:
        ...
        if man is not None:
            ...
            report += "\t\t\t<td style=\"font-weight: bold;\">" + man.get_sdk_version()["target"] if "target" in man.get_sdk_version() else "" + "</td>\n"  # noqa
            report += "\t\t</tr>\n"
            ...
            for permission in man.get_permissions():
                if permission == "android.permission.SEND_SMS" or \
                        permission == "android.permission.RECEIVE_SMS" or \
                        permission == "android.permission.RECEIVE_MMS" or \
                        permission == "android.permission.CALL_PHONE" or \
                        permission == "android.permission.CALL_PRIVILEGED" or \
                        permission == "android.permission.PROCESS_OUTGOING_CALLS" or \
                        permission == "android.permission.INSTALL_PACKAGES" or \
                        permission == "android.permission.MOUNT_FORMAT_FILESYSTEMS" or \
        ...
        if cert is not None:
            ...
            report += "\t\t\t<td style=\"font-weight: bold;\">From: " + cert.get_validity()["from"] + " - Until: " + cert.get_validity()["until"] + "</td>\n"  # noqa
            report += "\t\t</tr>\n"
            report += "\t\t<tr>\n"
